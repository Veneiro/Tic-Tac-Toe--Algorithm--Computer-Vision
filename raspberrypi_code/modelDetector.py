...
def _estimar_homografia_desde_celdas(celdas_centros, celdas_boxes):
    ...
    # Siempre usar rectángulo orientado para evitar diagonales cruzadas
    rect = cv2.minAreaRect(pts)
    box = cv2.boxPoints(rect)
    src = _ordenar_puntos(box)

    # Homografía preliminar para analizar cuántas filas/columnas completas hay
    ...
    # Expandir área cuando falten celdas para no recortar esquinas del tablero
    ...
    # Construcción del polígono expandido por eje (u/v) para no dejar fuera fila/columna faltante
    tl, tr, br, bl = src
    eje_x = tr - tl
    eje_y = bl - tl
