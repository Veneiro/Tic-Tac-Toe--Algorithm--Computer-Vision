...
# ==========================================
# AJUSTES RÁPIDOS
# Subir thresholds y profundidad mejora precisión pero hace el sistema más lento.
# ==========================================

AJUSTES = {
    # Detección / postprocesado
    'conf_min_roja': 0.45,
    'conf_min_azul': 0.36,
    ...
    # IA: bajar temp = más determinista; subir depth = más fuerte pero más lento
    't_softmax': 0.5,
    'search_depth': 2,
}
...
CONF_MIN_ROJA       = AJUSTES['conf_min_roja']
CONF_MIN_AZUL       = AJUSTES['conf_min_azul']
CONF_MIN_AZUL_PALIDA = AJUSTES['conf_min_azul_palida']

# Geometría del tablero
DIST_DUPLICADO_FALLBACK  = AJUSTES['dist_duplicado_fallback']
MARGEN_INTERIOR_NORM     = AJUSTES['margen_interior_norm']
RADIO_CENTRO_CELDA_NORM  = AJUSTES['radio_centro_celda_norm']
SHRINK_FALLBACK          = AJUSTES['shrink_fallback']
ANCLA_Y_FICHA_NORM       = AJUSTES['ancla_y_ficha_norm']
PESO_ANCLA_FILA          = AJUSTES['peso_ancla_fila']
PESO_ANCLA_COLUMNA       = AJUSTES['peso_ancla_columna']

# Calidad de imagen
UMBRAL_BLUR_LAPLACIAN    = AJUSTES['umbral_blur_laplacian']
UMBRAL_SATURACION_MEDIA  = AJUSTES['umbral_saturacion_media']
FACTOR_CONTRASTE_PALIDO  = AJUSTES['factor_contraste_palido']
FACTOR_SATURACION_PALIDO = AJUSTES['factor_saturacion_palido']

# Expansión del área de búsqueda
EXPANSION_AREA_BASE             = AJUSTES['expansion_area_base']
EXPANSION_AREA_POR_CELDA_FALTANTE = AJUSTES['expansion_area_por_celda_faltante']
EXPANSION_AREA_MAX              = AJUSTES['expansion_area_max']
TOL_NIVELES_EJE                 = AJUSTES['tol_niveles_eje']

# IA
T_SOFTMAX           = AJUSTES['t_softmax']
SEARCH_DEPTH        = AJUSTES['search_depth']
...
def _validar_matriz_resultante(matriz: np.ndarray):
    """Rechaza tableros imposibles o demasiado ambiguos para evitar falsas jugadas."""
    if matriz.shape != (3, 3):
        return False, 'tamano_invalido'

    if not np.isin(matriz, [0, 1, 2]).all():
        return False, 'valores_invalidos'

    conteo_x = int(np.sum(matriz == 1))
    conteo_o = int(np.sum(matriz == 2))

    if conteo_x > 5 or conteo_o > 5:
      return False, 'demasiadas_fichas'

    if abs(conteo_x - conteo_o) > 1:
        return False, 'conteo_imposible'
...
def _procesar_imagen_modelo(img: np.ndarray, model=None):
    ...
    if H is not None:
        ...
        if matriz_fb is None:
            ...
            valido, motivo = _validar_matriz_resultante(matriz_fb)
            if not valido:
                resultado_str = f'tablero={{Error: tablero_invalido_{motivo}}}'
            else:
    ...
    valido, motivo = _validar_matriz_resultante(matriz if H is not None else (matriz_fb if len(celdas_centros) >= 5 and 'matriz_fb' in locals() and matriz_fb is not None else np.zeros((3, 3), dtype=int)))
    if not valido and 'Error:' not in resultado_str:
        resultado_str = f'tablero={{Error: tablero_invalido_{motivo}}}'
