...
# ==========================================
# MODELO (singleton) + estado global
# ==========================================
...
# CLAHE creado una vez (solo guarda parámetros, .apply() es stateless → thread-safe)
_CLAHE = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(8, 8))

# Lock que serializa la inferencia YOLO. Con threaded=True Flask lanza un hilo
# por petición; sin lock dos hilos podrían invocar el modelo a la vez, lo que
# en GPU provoca contención y en CPU eleva el consumo de memoria.
_LOCK_INFERENCIA = threading.Lock()


def _obtener_modelo() -> YOLO:
    global _MODEL, _ID_ROJA, _ID_AZUL, _ID_CELDA
    if _MODEL is None:
        print('Cargando modelo...')
        _MODEL   = YOLO(RUTA_MODELO)
        clases   = _MODEL.names if hasattr(_MODEL, 'names') else {}
        _ID_ROJA  = next((k for k, v in clases.items() if v == NOMBRE_ROJA),  -1)
        _ID_AZUL  = next((k for k, v in clases.items() if v == NOMBRE_AZUL),  -1)
        _ID_CELDA = next((k for k, v in clases.items() if v == NOMBRE_CELDA), -1)
    return _MODEL
...
def _inferir_mejor_resultado(model, img: np.ndarray, id_roja: int, id_azul: int, id_celda: int):
    """Infiere una sola vez sobre la imagen original para minimizar latencia."""
    blur_score, sat_media = _metrica_calidad(img)
    resultado = model(img, conf=0.25, iou=0.5, imgsz=640)[0]
    return 'original', resultado, blur_score, sat_media
...
def _procesar_imagen_modelo(img: np.ndarray, model=None, annotate: bool = True):
    """Pipeline completo: inferencia → asignación de celdas → anotación opcional."""
    if img is None:
        raise ValueError('La imagen recibida es None')

    model = model or _obtener_modelo()

    # Pre-reducir imágenes grandes antes de pasarlas a YOLO.
    # YOLO internamente usa 640 px, pero el decode y la copia de memoria son
    # proporcionales a la resolución original del ESP32-CAM (hasta 1600×1200).
    h, w = img.shape[:2]
    if max(h, w) > 960:
        scale = 960 / max(h, w)
    ...
    # IDs de clase (cacheados en _obtener_modelo; variables locales por legibilidad)
    id_roja, id_azul, id_celda = _ID_ROJA, _ID_AZUL, _ID_CELDA
    ...
    if H is not None:
        ...
        resultado_str = ('tablero={Error: Zona de juego invalida}'
                         if matriz_fb is None else _matriz_a_str(matriz_fb))
    else:
        modo_zona     = 'fallback'
        resultado_str = 'tablero={Error: Pocas celdas detectadas}'

    print(f'[{modo_zona}] {resultado_str} | '
          f'celdas={len(celdas_centros)} fichas={len(fichas_finales)} '
          f'blur={blur_score:.0f} sat={sat_media:.0f}')

    # resultados.plot() renderiza bounding-boxes sobre la imagen (~50-100 ms).
    # Solo se ejecuta cuando el llamador necesita la imagen anotada (Gradio/web).
    img_anotada = (
        _anotar_imagen(resultados.plot(), tablero_poly, celdas_centros, bbox)
...
def analizar_tablero_esp32(img: np.ndarray) -> str:
    # annotate=False: no genera imagen anotada, ahorra ~50-100 ms por petición
    resultado_str, _, _ = _procesar_imagen_modelo(img, annotate=False)
