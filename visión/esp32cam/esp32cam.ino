#include "esp_camera.h"
#include <WiFi.h>
#include <WebServer.h>

// ── Configuración ──────────────────────────
const char *SSID     = "Galaxy S23 E57C";
const char *PASSWORD = "2ppyahxwjx4g7zu";

// ── Pines AI Thinker ESP32-CAM ─────────────
#define PWDN_GPIO_NUM  32
#define RESET_GPIO_NUM -1
#define XCLK_GPIO_NUM   0
#define SIOD_GPIO_NUM  26
#define SIOC_GPIO_NUM  27
#define Y9_GPIO_NUM    35
#define Y8_GPIO_NUM    34
#define Y7_GPIO_NUM    39
#define Y6_GPIO_NUM    36
#define Y5_GPIO_NUM    21
#define Y4_GPIO_NUM    19
#define Y3_GPIO_NUM    18
#define Y2_GPIO_NUM     5
#define VSYNC_GPIO_NUM 25
#define HREF_GPIO_NUM  23
#define PCLK_GPIO_NUM  22
#define LED_GPIO_NUM    4  // flash LED integrado

WebServer server(80);

// GET /capturar → devuelve un JPEG
void handleCapturar() {
  camera_fb_t* fb = esp_camera_fb_get();
  if (!fb) {
    server.send(500, "text/plain", "Error al capturar");
    return;
  }
  server.sendHeader("Access-Control-Allow-Origin", "*");
  server.send_P(200, "image/jpeg", (const char*)fb->buf, fb->len);
  esp_camera_fb_return(fb);
}

// GET / → estado básico
void handleRoot() {
  String ip = WiFi.localIP().toString();
  server.send(200, "text/plain",
    "ESP32-CAM activa\nEndpoint: http://" + ip + "/capturar");
}

void initCamera() {
  camera_config_t cfg;
  cfg.ledc_channel = LEDC_CHANNEL_0;
  cfg.ledc_timer   = LEDC_TIMER_0;
  cfg.pin_d0       = Y2_GPIO_NUM;
  cfg.pin_d1       = Y3_GPIO_NUM;
  cfg.pin_d2       = Y4_GPIO_NUM;
  cfg.pin_d3       = Y5_GPIO_NUM;
  cfg.pin_d4       = Y6_GPIO_NUM;
  cfg.pin_d5       = Y7_GPIO_NUM;
  cfg.pin_d6       = Y8_GPIO_NUM;
  cfg.pin_d7       = Y9_GPIO_NUM;
  cfg.pin_xclk     = XCLK_GPIO_NUM;
  cfg.pin_pclk     = PCLK_GPIO_NUM;
  cfg.pin_vsync    = VSYNC_GPIO_NUM;
  cfg.pin_href     = HREF_GPIO_NUM;
  cfg.pin_sscb_sda = SIOD_GPIO_NUM;
  cfg.pin_sscb_scl = SIOC_GPIO_NUM;
  cfg.pin_pwdn     = PWDN_GPIO_NUM;
  cfg.pin_reset    = RESET_GPIO_NUM;
  cfg.xclk_freq_hz = 20000000;
  cfg.pixel_format = PIXFORMAT_JPEG;

  if (psramFound()) {
    cfg.frame_size   = FRAMESIZE_VGA;  // 640×480
    cfg.jpeg_quality = 10;             // 0–63, menor = mayor calidad
    cfg.fb_count     = 2;
  } else {
    cfg.frame_size   = FRAMESIZE_CIF;  // 400×296
    cfg.jpeg_quality = 12;
    cfg.fb_count     = 1;
  }

  if (esp_camera_init(&cfg) != ESP_OK) {
    Serial.println("Error: no se pudo inicializar la cámara");
    while (true) { delay(1000); }
  }

  // Ajustes opcionales de imagen
  sensor_t* s = esp_camera_sensor_get();
  s->set_brightness(s, 0);    // -2 a 2
  s->set_contrast(s, 0);      // -2 a 2
  s->set_saturation(s, 0);    // -2 a 2
  s->set_whitebal(s, 1);      // auto white balance
  s->set_exposure_ctrl(s, 1); // auto exposición
  s->set_hmirror(s, 0);       // espejo horizontal (1 = activado)
  s->set_vflip(s, 0);         // flip vertical   (1 = activado)
}

void setup() {
  Serial.begin(115200);

  pinMode(LED_GPIO_NUM, OUTPUT);
  digitalWrite(LED_GPIO_NUM, LOW); // LED apagado al inicio

  initCamera();

  WiFi.begin(SSID, PASSWORD);
  Serial.print("Conectando a WiFi");
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println("\n=== LISTO ===");
  Serial.print("URL: http://");
  Serial.print(WiFi.localIP());
  Serial.println("/capturar");

  server.on("/",         HTTP_GET, handleRoot);
  server.on("/capturar", HTTP_GET, handleCapturar);
  server.begin();

  // Parpadeo de confirmación
  for (int i = 0; i < 3; i++) {
    digitalWrite(LED_GPIO_NUM, HIGH); delay(100);
    digitalWrite(LED_GPIO_NUM, LOW);  delay(100);
  }
}

void loop() {
  server.handleClient();
}
