...
String raspberryURL = "http://192.168.100.122:5000/procesar"; 
String esp32s3URL = "http://192.168.100.120/tablero"; // Cambia esta IP por la del ESP32-S3
...
void sendBoardToESP32S3(const String& boardState) {
  ...
  if (forwardHttp.begin(esp32s3URL)) {
    forwardHttp.addHeader("Content-Type", "text/plain; charset=utf-8");
...
void captureAndSend() {
  ...
  // 1. Encender LED Blanco
  // Nota: En algunos ESP-EYE es HIGH, en otros LOW. Prueba HIGH primero.
  digitalWrite(LED_PIN, HIGH); 
  delay(200); // Pequeña pausa para que la cámara ajuste la exposición a la nueva luz

  // 2. Capturar Foto (Tomamos el control de la cámara un momento)
  camera_fb_t * fb = esp_camera_fb_get();
  
  // 3. Apagar LED (Ya tenemos la foto)
  ...
  // 4. Enviar a Raspberry Pi
  HTTPClient http;
  Serial.print("Conectando a Raspberry: "); Serial.println(raspberryURL);
  ...
  if (http.begin(raspberryURL)) {
    ...
    if (httpCode > 0) {
      ...
      Serial.println(payload); // AQUÍ SALE EL STRING DEL TABLERO
      Serial.println("-----------------------------\n");

      // Reenviar el tablero al ESP32-S3 para visualizarlo por Serial
      sendBoardToESP32S3(payload);
    } else {
      Serial.printf("Error HTTP: %s\n", http.errorToString(httpCode).c_str());
    }
    http.end();
  } else {
    Serial.println("No se pudo conectar con la Raspberry");
  }
