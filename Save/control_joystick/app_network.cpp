...
void procesarEntradaTablero(const String &entrada)
{
  ...
  if (entrada.indexOf("\"tablero\"") >= 0)
  {
    ...
    Serial.printf("[RX] fuera_rojo=%d  fuera_azul=%d\n", fuera_rojo, fuera_azul);
  }
  ...
  // Si hay ganador, mostramos la copa y evitamos ejecutar cualquier movimiento.
  ...
  if (ganadorDetectado != 0)
  {
    ...
    // Enviar el estado a Raspberry si está configurado para reenviar
