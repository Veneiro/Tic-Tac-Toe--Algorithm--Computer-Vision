...
COLOR_THRESH = 0.040   # fracción del área de la celda
...
def warp_panel(img, debug=False):
    ...
    # Cerrar huecos de las líneas negras de la cuadrícula
    k = cv2.getStructuringElement(cv2.MORPH_RECT, (15, 15))
    white_closed = cv2.morphologyEx(white, cv2.MORPH_CLOSE, k)
    white_clean  = cv2.morphologyEx(white_closed,
                                    cv2.MORPH_OPEN,
                                    cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)))

    # Encontrar contorno del panel (blob blanco grande y más cuadrado posible)
...
def find_grid_bbox(warped, debug=False):
    ...
    Criterios para seleccionar el contorno correcto:
      - Interior predominantemente blanco  (las celdas)
      - Razonablemente cuadrado
      - Lo más centrado posible en el panel
    """
    side = warped.shape[0]
    gray = cv2.cvtColor(warped, cv2.COLOR_BGR2GRAY)

    # Threshold bajo para capturar solo el negro intenso
    _, black = cv2.threshold(gray, 70, 255, cv2.THRESH_BINARY_INV)

    # Eliminar ruido mínimo
    k_open = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
    black  = cv2.morphologyEx(black, cv2.MORPH_OPEN, k_open)

    # Dilatar para unir segmentos de las líneas
    ...
    for cnt in cnts[:10]:
        ...
        if min(gw, gh) / max(gw, gh) < 0.45:
            continue

        # Fracción de blanco en el interior
        ...
        cx = gx + gw / 2
        cy = gy + gh / 2
        dist = ((cx - side / 2) ** 2 + (cy - side / 2) ** 2) ** 0.5
        centrality = max(0.0, 1.0 - dist / (side * 0.7))

        # Penalizar tamaño excesivo (que sería todo el panel)
        size_frac = area / (side ** 2)
        size_score = max(0.1, 1.0 - abs(size_frac - 0.35) / 0.35)

        ratio = min(gw, gh) / max(gw, gh)
        score = ratio * wf * centrality * size_score
