...
float toEncoderFrame(float q_art, float q0, bool invertDirection){
    ...
    if(invertDirection)
        q_enc = -q_art + q0;   // encoder gira al revés
    else
        q_enc =  q_art + q0;   // encoder gira igual
...
float fromEncoderFrame(float q_enc, float q0, bool invertDirection){
    ...
    if(invertDirection)
        q_art = -(q_enc - q0);   // encoder gira al revés
    else
        q_art =  (q_enc - q0);   // encoder gira igual
